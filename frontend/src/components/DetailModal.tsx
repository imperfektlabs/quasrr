            ...
            <div key={seasonNumber} className="glass-card rounded-md px-3 py-2 text-sm">
              ...
              {isExpanded && (
                <div className="mt-2 space-y-1 text-sm text-slate-300">
                  ...
                  {episodes.map((ep) => {
                    const qualityLabel = ep.quality || (ep.hasFile ? 'Quality unknown' : 'Missing')
                    return (
                      <div key={ep.id} className="flex items-center justify-between">
                        <span className="truncate">
                          ...
                          {ep.airDate && (
                            <span className="ml-2 text-slate-500">{ep.airDate}</span>
                          )}
                        </span>
                        <span className="text-slate-500">{qualityLabel}</span>
                      </div>
                    )
                  })}
                </div>
