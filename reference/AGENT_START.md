...
1. **Primary reference material** (recursive):
   ...
   ```
   [REDACTED_PATH]/live/quasrr/reference/
   ```
2. **Agent conventions & constraints**:

   ```
   [REDACTED_PATH]/live/AGENTS.md
   ```
...
To test the build on the NAS:
   ...
   ```
   [REDACTED_PATH]/docs/scripts/smoke.sh quasrr --action up-build --down
   ```
   This will:
